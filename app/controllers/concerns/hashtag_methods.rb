...
module HashtagMethods
    ...
    def extract_hashtag(body)
      if body.blank? #例外処理のため。引数が空で渡ってきた場合は処理をしない
        return
      end
    ...
    def save_hashtag(hashtag_array,post_instance)
      ...
      hashtag_array.uniq.map do |hashtag|
        ...
        #中間テーブルへの保存処理
        book_hashtag = BookHash.new #中間テーブルのインスタンスを作成
        book_hashtag.book_id = post_instance.id
        book_hashtag.hashtag_id = tag.id
        book_hashtag.save!
      end
    end
    ...
    def delete_records_related_to_hashtag(book_id)
      relationship_records = BookHash.where(book_id: book_id) #中間テーブルのレコード
      ...
      all_related_records = BookHash.all
